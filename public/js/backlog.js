        ...
        if ((potentialAddingZone.find('.issue-root').length > 0)) {
            $.get(`/project/kose/issue/newform`, function (data) {
                ...
                form.find('.send-button').on('click', (e) => {
                    ...
                    $.post('/project/kose/issue/new', {parent: parentKey, type: issueType, status: issueStatus, title: issueTitle, estimation: issueEstimation})
                    //console.log(issueType, issueStatus, issueTitle, issueEstimation)
                })
            })
        } else {
